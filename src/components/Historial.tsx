...
export default function Historial() {
  ...
  const [registroSeleccionado, setRegistroSeleccionado] = useState<RegistroPomodoro | null>(null);
          ...
          {fechasOrdenadas.map((fecha) => (
            ...
            >
              ...
              <div className="overflow-x-auto">
                <table className="table w-full table-fixed text-custom-text text-left">
                  <thead>
                    <tr className="border-b border-white/5 text-custom-text uppercase text-xs">
                      <th className="font-semibold w-1/4">{t("historial.tabla.modo")}</th>
                      <th className="font-semibold w-1/4">{t("historial.tabla.estado")}</th>
                      <th className="font-semibold w-1/4">{t("historial.tabla.estudio")}</th>
                      <th className="font-semibold w-1/4 text-right">{t("historial.tabla.acciones")}</th>
                    </tr>
                  </thead>
                  <tbody>
...
gap-2 text-green-500 text-sm font-medium"
                              >
                                <FaCheckCircle />
                                {t("historial.estado.completado")}
                              </span>
                            ) : (
                              <span
                                className="flex items-center     
gap-2 text-yellow-500 text-sm font-medium"
                              >
                                <FaTimesCircle />
                                {t("historial.estado.abortado")}
                              </span>
                            )}
                          </td>
                          <td
                            className="text-[#EC4166]              
font-bold"
                          >
                            {registro.minutos}m
                          </td>
                          <td className="flex justify-end gap-2">
                            <button
                              ...
                              className="p-3 rounded-xl bg-custom-bg 
text-custom-text/50 hover:text-[#72c1d9] border border-white/5     
transition-all"
                              title="Editar"
                            >
                              <FaPen size={14} />
      ...
      {registroSeleccionado && (
        <ModalEditarRegistro
          ...
          valorInicial={registroSeleccionado.minutos}
          cerrarModal={() => {
            setModalAbierto(false);
            setRegistroSeleccionado(null);
          }}
          alGuardar={(nuevosMins) => {
            editarRegistro(registroSeleccionado.id, nuevosMins);
